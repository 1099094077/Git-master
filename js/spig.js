...
jQuery(document).ready(function ($) {
    $("#spig").mousedown(function (e) {
        if(e.which==3){
        showmessagexr("秘密通道:<br />    <a href=\"http://blog.qianc.me\" title=\"首页\">首页</a> | <a href=\"http://blog.qianc/wp-admin/\" title=\"后台\">后台</a> ",10000);
}
});
...
//鼠标在消息上时
jQuery(document).ready(function ($) {
    $("#messagexr").hover(function () {
       $("#messagexr").fadeTo("100", 1);
     });
});
...
jQuery(document).ready(function ($) {
    if (isindex) { //如果是主页
        ...
        if (now > 0 && now <= 6) {
            showmessagexr('欧尼酱，你是夜猫子呀？还不睡觉，明天起的来么你？', 6000);
        } else if (now > 6 && now <= 11) {
            showmessagexr('欧尼酱，早上好，早起的鸟儿有虫吃噢！早起的虫儿被鸟吃，你是鸟儿还是虫儿？嘻嘻！', 6000);
        } else if (now > 11 && now <= 14) {
            showmessagexr('欧尼酱，中午了，吃饭了么？不要饿着了，饿死了谁来挺我呀！', 6000);
        } else if (now > 14 && now <= 18) {
            showmessagexr('欧尼酱，中午的时光真难熬！还好有你在！', 6000);
        } else {
            showmessagexr('欧尼酱，快来逗我玩吧！', 6000);
        }
    }
    else {
        showmessagexr('欢迎您来到祭夜blog阅读 ' + title + ' ', 6000);
    }
    $(".spig").animate({
...
//        showmessagexr("下面播报明日天气<iframe name=\"xidie\" src=\"http://api.lwl12.com/hitokoto/?encode=js\"frameborder=\“0\” scrolling=\"no\"  width=\"130px\" allowtransparency=\"true\" ></iframe>", 10000);
//    },
//	4000);
});
...
jQuery(document).ready(function ($) {
    $('h2 a').click(function () {//标题被点击时
        showmessagexr('正在用吃奶的劲加载《<span style="color:#0099cc;">' + $(this).text() + '</span>》请稍候');
    });
    $('h2 a').mouseover(function () {
        showmessagexr('要看看《<span style="color:#0099cc;">' + $(this).text() + '</span>》这篇文章么？');
    });
    $('#prev-page').mouseover(function(){
        showmessagexr('要翻到上一页吗?');
    });
    $('#next-page').mouseover(function(){
        showmessagexr('要翻到下一页吗?');
    });
    $('#index-links li a').mouseover(function () {
        showmessagexr('去 <span style="color:#0099cc;">' + $(this).text() + '</span> 逛逛');
    });
    $('.comments').mouseover(function () {
        showmessagexr('<span style="color:#0099cc;">' + visitor + '</span> 向评论栏出发吧！');
    });
    $('#submit').mouseover(function () {
        showmessagexr('确认提交了么？');
    });
    $('#s').focus(function () {
        showmessagexr('输入你想搜索的关键词再按Enter(回车)键就可以搜索啦!');
    });
    $('#go-prev').mouseover(function () {
        showmessagexr('点它可以后退哦！');
    });
    $('#go-next').mouseover(function () {
        showmessagexr('点它可以前进哦！');
    });
    $('#refresh').mouseover(function () {
        showmessagexr('点它可以重新载入此页哦！');
    });
    $('#go-home').mouseover(function () {
        showmessagexr('点它就可以回到首页啦！');
    });
    $('#addfav').mouseover(function () {
        showmessagexr('点它可以把此页加入书签哦！');
    });
    $('#nav-two a').mouseover(function () {
        showmessagexr('嘘，从这里可以进入控制面板的哦！');
    });
    $('.post-category a').mouseover(function () {
        showmessagexr('点击查看此分类下得所有文章');
    });
    $('.post-heat a').mouseover(function () {
        showmessagexr('点它可以直接跳到评论列表处.');
    });
    $('#tho-shareto span a').mouseover(function () {
        showmessagexr('你知道吗?点它可以分享本文到'+$(this).attr('title'));
    });
    $('#switch-to-wap').mouseover(function(){
        showmessagexr('点击可以切换到手机版博客版面');
    });
});
...
jQuery(document).ready(function ($) {
    ...
    $("#email").click(function () {
        showmessagexr("留下你的邮箱，不然就是无头像人士了！");
        $(".spig").animate({
    ...
    $("#url").click(function () {
        ...
        showmessagexr("快快告诉我你的家在哪里，好让我去参观参观！");
        $(".spig").animate({
            top: $("#url").offset().top - 170,
            left: $("#url").offset().left - 80
        },
		{
		    queue: false,
		    duration: 1000
    ...
    $("#comment").click(function () {
        showmessagexr("认真填写哦！不然会被认作垃圾评论的！我的乖乖~");
        $(".spig").animate({
            top: $("#comment").offset().top - 170,
            left: $("#comment").offset().left - 10
        },
		{
		    queue: false,
		    duration: 1000
...
//显示消息函数 
function showmessagexr(a, b) {
    if (b == null) b = 10000;
    jQuery("#messagexr").hide().stop();
    jQuery("#messagexr").html(a);
    jQuery("#messagexr").fadeIn();
    jQuery("#messagexr").fadeTo("1", 1);
    jQuery("#messagexr").fadeOut(b);
};
